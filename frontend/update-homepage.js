    ...
    <div className="space-y-6 animate-in fade-in duration-500 w-full">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Tuỳ chỉnh nội dung hiển thị trên trang chủ (Đồng bộ theo luồng người dùng).</p>
        </div>
        <button 
          onClick={handleSave}
          disabled={isSaving}
      ...
      {successMessage && (
        <div className="bg-emerald-50 text-emerald-600 p-4 rounded-lg text-sm font-medium border border-emerald-200 flex items-center shadow-sm">
      ...
      <div className="space-y-4">
        {sections.map(sec => (
          <div key={sec.id} className="bg-white dark:bg-[#14151a] rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden shadow-sm">
            <div 
              className="px-6 py-4 flex items-center justify-between cursor-pointer hover:bg-gray-50 dark:hover:bg-[#1a1b23] transition-colors"
            ...
            >
              ...
              {expandedSection === sec.id ? <ChevronUp className="w-5 h-5 text-gray-500" /> : <ChevronDown className="w-5 h-5 text-gray-500" />}
            </div>
            
            {expandedSection === sec.id && (
              <div className="p-6 border-t border-gray-200 dark:border-gray-800 space-y-5 bg-gray-50/30 dark:bg-transparent">
                {sec.id === 'seo' ? (
                  <>
                    <div>
...
fs.writeFileSync('d:/Hieudeptraivl/noithat-main/frontend/src/app/admin/(dashboard)/homepage/page.tsx', content);
