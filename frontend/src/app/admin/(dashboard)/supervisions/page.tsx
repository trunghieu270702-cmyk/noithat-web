...
export default function SupervisionsPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    confirm({
      ...
      onConfirm: async () => {
        try {
      await Promise.all(selectedIds.map(id => apiClient.delete(`/supervisions/${id}`)));
      setSelectedIds([]);
      fetchSupervisions();
...
const handleDelete = async (id: number) => {
    confirm({
      title: 'Xác nhận xóa',
      description: 'Bạn có chắc chắn muốn xóa gói giám sát này?',
      variant: 'danger',
      onConfirm: async () => {
        try {
            ...
            <div className="mt-4 grid grid-cols-2 gap-3">
              <div className="p-3.5 rounded-[4px] border border-emerald-100 dark:border-emerald-900/30 bg-emerald-50/50 dark:bg-emerald-500/10">
                <div className="flex items-center gap-2 mb-1.5">
                  <div className="w-2 h-2 bg-emerald-50 dark:bg-emerald-500/100 rounded-[4px]"></div><span className="text-xs font-medium text-gray-700 dark:text-gray-300">Hoạt động</span>
                </div>
                ...
                currentData.map((sup, index) => (
                  <tr key={sup.id} className="border-b border-gray-200 dark:border-gray-800 hover:bg-gray-50/50 dark:hover:bg-[#262930] dark:bg-[#1a1b23] transition-colors group animate-in fade-in slide-in-from-bottom-4 duration-500" style={{ animationDelay: `${index * 50}ms`, animationFillMode: 'both' }}>
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-4 h-4 rounded-[4px] border flex items-center justify-center cursor-pointer transition-colors shrink-0 ${selectedIds.includes(sup.id.toString()) ? 'bg-[#5865f2] border-[#5865f2]' : 'border-gray-300'
                            }`}
                        ...
                        <div>
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="px-2 py-0.5 rounded-[4px] text-[10px] font-medium bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 border border-gray-200 dark:border-gray-800">{sup.packageType}</span>
                          </div>
                    ...
                    <td className="px-5 py-3.5 border-l border-gray-200 dark:border-gray-800">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-[4px] text-xs font-medium ${sup.status === 'ACTIVE' ? 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/20' : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-800'
                        }`}>
        ...
        <div className="fixed inset-0 z-[100] flex justify-end">
          ...
          <div className="relative bg-white dark:bg-[#14151a] w-full max-w-2xl h-full flex flex-col border-l border-gray-200 dark:border-gray-800 animate-in slide-in-from-right duration-300 shadow-none z-10">
              ...
              <form id="supervision-form" onSubmit={handleSave} className="p-6 space-y-8">
                ...
                <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-2 gap-5">
                    <div className="space-y-1.5">
                      ...
                      <div className="relative">
                        ...
                        <input type="text" value={formData.packageName} onChange={e => { setFormData({ ...formData, packageName: e.target.value }); if (errors.packageName) setErrors({ ...errors, packageName: '' }); }} className={`pl-9 w-full bg-gray-50/50 dark:bg-[#1a1b23] border ${errors.packageName ? 'border-red-500 focus:ring-red-500/20' : 'border-gray-200 dark:border-gray-700 focus:ring-[#5865f2]/20'} text-sm h-10 rounded-[4px] text-gray-900 dark:text-white transition-all hover:bg-white dark:bg-[#14151a] dark:hover:bg-[#1a1b23] focus:outline-none focus:ring-[3px] focus:border-[#5865f2]/40`} placeholder="VD: Gói Giám Sát Toàn Diện..." />
                      </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <textarea value={formData.shortDescription} onChange={e => setFormData({ ...formData, shortDescription: e.target.value })} className="w-full bg-gray-50/50 dark:bg-[#1a1b23] border border-gray-200 dark:border-gray-700 text-sm rounded-[4px] text-gray-900 dark:text-white p-3 transition-all hover:bg-white dark:bg-[#14151a] dark:hover:bg-[#1a1b23] focus:outline-none focus:ring-[3px] focus:ring-[#5865f2]/20 focus:border-[#5865f2]/40 resize-none h-20" placeholder="Mô tả về gói giám sát..." />
                  </div>
                </div>
                ...
                <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-2 gap-5">
                    <div className="space-y-1.5">
                      ...
                      <div className="relative">
                        ...
                        <input type="text" value={formData.feeStructure} onChange={e => setFormData({ ...formData, feeStructure: e.target.value })} className="pl-9 w-full bg-gray-50/50 dark:bg-[#1a1b23] border border-gray-200 dark:border-gray-700 text-sm h-10 rounded-[4px] text-gray-900 dark:text-white transition-all hover:bg-white dark:bg-[#14151a] dark:hover:bg-[#1a1b23] focus:outline-none focus:ring-[3px] focus:ring-[#5865f2]/20 focus:border-[#5865f2]/40" placeholder="VD: 1.000.000đ/tháng" />
                      </div>
                    </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <textarea value={formData.scopeOfWork} onChange={e => setFormData({ ...formData, scopeOfWork: e.target.value })} className="w-full bg-gray-50/50 dark:bg-[#1a1b23] border border-gray-200 dark:border-gray-700 text-sm rounded-[4px] text-gray-900 dark:text-white p-3 transition-all hover:bg-white dark:bg-[#14151a] dark:hover:bg-[#1a1b23] focus:outline-none focus:ring-[3px] focus:ring-[#5865f2]/20 focus:border-[#5865f2]/40 resize-none h-24" placeholder="Chi tiết phạm vi công việc..." />
                  </div>
                </div>
              </form>
            </div>
            ...
            <div className="h-20 flex items-center justify-end gap-3 px-6 border-t border-gray-100 dark:border-gray-800 bg-white dark:bg-[#14151a] shrink-0">
              ...
              <button type="submit" form="supervision-form" className="flex items-center gap-2 bg-[#5865f2] hover:bg-[#4752c4] text-white rounded-[4px] font-medium text-sm h-10 px-6 border-0 cursor-pointer transition-colors">
