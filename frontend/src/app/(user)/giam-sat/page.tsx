    ...
    <div className="pt-[120px] pb-20 bg-[#131313] min-h-screen text-white">
      <div className="container mx-auto px-6 max-w-[1200px]">
        ...
        <div className="text-center mb-20 max-w-4xl mx-auto">
          <h1 className="text-4xl md:text-5xl font-['Montserrat',_sans-serif] font-bold text-white mb-6">Dịch vụ giám sát thi công nội thất</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-20">
          {isLoading ? (
            <div className="col-span-2 text-center text-white/50">Đang tải các gói dịch vụ...</div>
          ) : packages.length > 0 ? packages.map((pkg, idx) => (
            <div key={pkg.id} className={`rounded-2xl border p-8 flex flex-col relative overflow-hidden transition-colors ${idx % 2 !== 0 ? 'bg-gradient-to-b from-[#ce9e51]/10 to-[#1c1c1c] border-[#ce9e51]/30 hover:border-[#ce9e51]' : 'bg-[#1c1c1c] border-white/10 hover:border-[#ce9e51]/50'}`}>
              {idx % 2 !== 0 && (
                <div className="absolute top-0 right-0 bg-[#ce9e51] text-white text-xs font-bold px-4 py-1 rounded-bl-lg uppercase tracking-wider">
                  Khuyên dùng
                </div>
              )}
              <div className="mb-6 pb-6 border-b border-white/10 mt-2">
              ...
              <div className="mb-8 flex-1">
                ...
                <ul className="space-y-2 text-sm text-white/80">
                  {(pkg.scopeOfWork?.split('\n') || ['Giám sát thi công nội thất']).map((item: string, i: number) => (
                    <li key={i}><i className="fa fa-arrow-right text-white/30 w-5"></i> {item}</li>
                  ))}
                </ul>
              </div>
              
              <Link href={`/tu-van?service=${pkg.packageName}`} className={`text-center block w-full font-bold py-4 px-6 rounded transition-colors uppercase tracking-wider text-sm ${idx % 2 !== 0 ? 'bg-[#ce9e51] hover:bg-white hover:text-[#131313] text-white' : 'bg-white/10 hover:bg-[#ce9e51] text-white'}`}>
                Đăng ký tư vấn
              </Link>
            </div>
          )) : (
            <div className="col-span-2 text-center text-white/50">Hiện tại chưa có gói dịch vụ nào.</div>
          )}
        </div>
          

        {/* Values */}
        <div className="bg-[#1c1c1c] rounded-2xl p-10 border border-white/5 text-center">
          <h2 className="text-2xl font-['Montserrat',_sans-serif] font-bold mb-10 text-white">Giá trị mang lại cho khách hàng</h2>
